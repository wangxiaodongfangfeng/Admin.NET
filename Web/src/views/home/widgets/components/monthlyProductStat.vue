...
export default {
	...
	description: '按产品类型统计指定年月的检测总数、OK数、NG数及合格率',
};
</script>

<script setup lang="ts" name="monthlyProductStat">
import { ref, onMounted } from 'vue';
import { Download } from '@element-plus/icons-vue';
import { getAPI } from '/@/utils/axios-utils';
import { LkStatisticsApi } from '/@/api-services/api';
import { LkProductTypeMonthlyStatOutput } from '/@/api-services/models';
...
const tableData = ref<LkProductTypeMonthlyStatOutput[]>([]);
...
const loadData = async () => {
	...
	try {
		...
		const res = await getAPI(LkStatisticsApi).apiLkStatisticsMonthlyProductTypeStatGet(year, month);
...
const handleExport = async () => {
	...
	try {
		const { year, month } = parseYearMonth(selectedMonth.value);
		const res = await getAPI(LkStatisticsApi).apiLkStatisticsExportMonthlyProductTypeStatPost(year, month);
		const url = URL.createObjectURL(new Blob([res.data as any]));
		const a = document.createElement('a');
		a.href = url;
		a.download = `产品类型月度统计_${selectedMonth.value}.xlsx`;
		a.click();
